...
export function printMedicalDocument(type, prescription, clinic, patient) {
  ...
  if (type === "report") {
    const body = `<section class="title"><h2>تقرير طبي</h2><p>ملخص حالة المريض · ${escapeHtml(prescription.id)}</p></section>${patientTiles}<section class="section"><h3>الملخص الطبي</h3><div class="section-content"><p class="diagnosis">التشخيص المسجل: ${escapeHtml(prescription.diagnosis)}</p><p class="note">هذا التقرير صادر بناءً على السجل الطبي المتاح لدى ${escapeHtml(clinic?.name || "العيادة")}، ويُستخدم للغرض الطبي والإداري المناسب.</p></div></section><section class="section"><h3>ملاحظات الطبيب</h3><div class="section-content"><div class="lines"></div></div></section><div class="signature"><div>الطبيب المعالج<br>${escapeHtml(prescription.doctor)}</div><div>ختم العيادة وتاريخ الاعتماد</div></div>`;
