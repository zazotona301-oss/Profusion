...
export function printMedicalRecordReport(record, clinic, patient) {
  const safePatient = patient || {};
  const signatureImage = clinic?.signature ? `<img class="signature-image" src="${escapeHtml(clinic.signature)}" alt="توقيع الطبيب">` : "";
  const patientTiles = `<div class="patient"><div class="tile"><span>المريض</span><strong>${escapeHtml(record.patientName)}</strong></div><div class="tile"><span>رقم الملف</span><strong>${escapeHtml(safePatient.id || record.patientId || "غير مسجل")}</strong></div><div class="tile"><span>العمر / النوع</span><strong>${escapeHtml(safePatient.age ? `${safePatient.age} سنة · ${safePatient.gender || ""}` : "غير مسجل")}</strong></div><div class="tile"><span>تاريخ الفحص</span><strong>${escapeHtml(new Intl.DateTimeFormat("ar-SA", { dateStyle: "medium" }).format(new Date(record.date)))}</strong></div></div>`;
  const vitalRows = Object.entries(record.vitalSigns || {}).filter(([, value]) => value).map(([key, value]) => `<tr><td>${escapeHtml(({ bloodPressure: "ضغط الدم", pulse: "النبض", temperature: "درجة الحرارة", weight: "الوزن", oxygen: "الأكسجين" })[key] || key)}</td><td>${escapeHtml(value)}</td></tr>`).join("");
  const section = (title, content) => `<section class="section"><h3>${title}</h3><div class="section-content">${content || "<span class=\"note\">غير مسجل</span>"}</div></section>`;
  const body = `<section class="title"><h2>تقرير الفحوصات الطبية</h2><p>السجل الطبي · ${escapeHtml(record.id)}</p></section>${patientTiles}${section("الشكوى الرئيسية والتشخيص", `<p class="diagnosis">الشكوى: ${escapeHtml(record.chiefComplaint || "غير مسجلة")}</p><p class="diagnosis">التشخيص: ${escapeHtml(record.diagnosis || "غير مسجل")}</p>`)}${section("العلامات الحيوية", vitalRows ? `<table class="meds"><thead><tr><th>القياس</th><th>النتيجة</th></tr></thead><tbody>${vitalRows}</tbody></table>` : "")}${section("نتيجة الفحص السريري", `<p>${escapeHtml(record.examination || "غير مسجل")}</p>`)}${section("نتائج التحاليل والفحوصات", `<p>${escapeHtml(record.labResults || "غير مسجلة")}</p>`)}${section("خطة العلاج والمتابعة", `<p>${escapeHtml(record.plan || "غير مسجلة")}</p><p><strong>الأدوية والتعليمات:</strong> ${escapeHtml(record.medications || "غير مسجلة")}</p>`)}<div class="signature"><div>${signatureImage}الطبيب المعالج<br>${escapeHtml(record.doctor || "غير مسجل")}</div><div>ختم العيادة وتاريخ الاعتماد</div></div>`;
  createPrintWindow("تقرير الفحوصات الطبية", clinic, body);
}
